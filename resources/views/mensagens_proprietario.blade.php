...
<head>
    ...
    <title>Coliving SAMCA - Mensagens Proprietário</title>
...
<body>
    ...
    <main>
        <div class="container custom-container" style="margin-top: 40px;">
            <div class="row">
                <div class="col s12">
                    <h4><i class="material-icons left samca-color-text">forum</i> Conversas com Estudantes</h4>
                    ...
                    <ul class="collection z-depth-1">
                        @forelse($conversations as $msg)
                            ...
                            <li class="collection-item avatar collection-item-message {{ $isUnread ? 'message-unread' : '' }} open-chat"
                                ...
                                <i class="material-icons circle orange darken-2">school</i>
                        ...
                        @empty
                            <li class="collection-item center-align" style="padding: 40px;">
                                <i class="material-icons grey-text large">mark_email_read</i>
        ...
        <div id="modal-chat" class="modal modal-fixed-footer">
            <div class="modal-content">
                <h5 class="samca-color-text"><i class="material-icons left">chat</i> <span id="chat-header-name">Conversa</span></h5>
                <div class="divider"></div>
                <br>
                <div id="chat-body">
                    <div class="center-align grey-text">Carregando...</div>
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.open-chat').forEach(item => {
                item.addEventListener('click', function() {
                    ...
                    document.getElementById('chat-body').innerHTML = '<div class="center-align" style="margin-top:20px;">Carregando...</div>';
            ...
            function carregarMensagens() {
                ...
                fetch(`/messages/history/${currentChatUserId}`)
                    ...
                    .then(data => {
                        ...
                        chatBody.innerHTML = ''; 
                        if(data.length === 0) {
                            chatBody.innerHTML = '<p class="center-align grey-text">Nenhuma mensagem.</p>';
                        } else {
                            data.forEach(msg => {
                                ...
                                chatBody.innerHTML += `<div class="chat-bubble ${classe}">${msg.body}<span class="chat-time">${dataMsg}</span></div>`;
                            });
                        }
            ...
            function enviarResposta() {
                ...
                let chatBody = document.getElementById('chat-body');
                chatBody.innerHTML += `<div class="chat-bubble chat-me" style="opacity: 0.7;">${texto} <span class="chat-time">...</span></div>`;
                chatBody.scrollTop = chatBody.scrollHeight;
                input.value = '';
                ...
                fetch("{{ route('messages.store') }}", {
                    ...
                    body: JSON.stringify({ to_id: currentChatUserId, body: texto })
                })
                .then(response => response.json())
